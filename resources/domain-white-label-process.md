...
Once we will have all the above information, we will order the SSL certificate for you from our partner [Digicert](https://www.digicert.com) and you will have to demonstrate the ownership over the domain. You will be assisted by one of our system engineers to validate the domain name by email or by a DNS TXT record and for the whole remaining process.
...
{% hint style="info" %}
We can start the white labelling process after the contract is signed and the production environment is activated from our side.
{% endhint %}
